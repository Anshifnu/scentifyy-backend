...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['id', 'user', 'shipping_info', 'payment_info', 'items', 'status', 'date']
        read_only_fields = ['user', 'status', 'date']

    def create(self, validated_data):
        # Extract nested data
        shipping_data = validated_data.pop('shipping_info')
        payment_data = validated_data.pop('payment_info')
        ...
        order = Order.objects.create(user=user, shipping_info=shipping, payment_info=payment, **validated_data)
